...
[[eingabefeld-vierAugen-Prinzip, ]]
==== 4-Augen-Prinzip Eingabefelder

Teilweise besteht bei Eingabefeldern die Notwendigket, dass die durch einen ersten Benutzer eingegebenen Daten, zunächst noch von einem zweiten Benutzer bestätigt/freigegeben werden müssen, bevor diese von anderen Aktionen (z.B. Zahlungsläufen) berücksichtigt werden können.
...
Zur Umsetzung von Anwendungsfällen, die auf dem 4-Augen-Prinzip beruhen, sind folgende Eingabefeld-Typen erforderlich:

*Ausprägungen*

* 4-Augen-Prinzip Eingabefeld
* 4-Augen-Prinzip Eingabefeld-Mehrzeilig
* 4-Augen-Prinzip Eingabefeld-Nummerischer-Wert
* 4-Augen-Prinzip Eingabefeld-Währung
* 4-Augen-Prinzip Eingabefeld-Datum
* 4-Augen-Prinzip Eingabefeld-Aktion
...
* Die möglichen Zustände (Normal, Deaktiviert, Focus, ...) entsprechen denen der jeweiligen (Eltern) Eingabefeld-Typen
...
* Der Zustand eines 4-Augen-Prinzip Eingabefeldes wird durch die Farbe des Schloss-Icon ausgedrückt (grau: zur Zeit kein Eingabewert | rot: Eingabewertfreigabe erforderlich)
...
Beim Betreten der Maske, vor der Eingabe von Werten im oberen Abschnittsbereich, waren die Schloss-Icons der Feler `Gültig ab` und `Stundensatz` ,grau dargestellt.
Nach der Eingabe eines Wertes änderte sich die jeweilige Schloss-Icon Farbe auf rot.
...
Nachdem Benutzer A zwei Stundensatzeinträge durch Drücken des `Hinzufügen` Buttons der Ergebnisliste hinzugefügt hat, kann er dann die somit modifizierte Ergebnisliste final abspeichern.
