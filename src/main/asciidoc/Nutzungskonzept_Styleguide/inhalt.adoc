...
Dieses Projekt baut auf dem Bootstrap CSS Framework in Version 3.3.6 auf.
...
[[einbindung-isy-style]]
== Einbindung der Bibliothek Isy-Style in eine Anwendung

Die Bibliothek Isy-Style wird wie jede andere Bibliothek der IsyFact in die `pom.xml` der Anwendung integriert.

Dazu ist folgende Abhängigkeit in die `pom.xml` einzutragen:

:desc-listing-einbindung_isy_style: Einbindung von Isy-Style
...
Während des Build-Prozesses der Anwendung werden die Style-Ressourcen (CSS, JavaScript, Bilder) automatisch aus `isy-style` geladen und in der Anwendung bereitgestellt.
Im Projekt selbst sind diese Ressourcen nicht sichtbar und können somit auch nicht geändert werden.
Änderungen an Styling-Ressourcen aus der `isy-style` sind somit nur durch ein Überschreiben der Ressourcen möglich.
In der Anwendung implementierte Styling-Ressourcen sollten immer nach den Ressourcen aus der `isy-style` geladen werden, damit diese überschrieben werden.

[[konfiguration-portalfarbe]]
== Konfiguration Portalfarbe

Für eine Anwendung kann eine Basisfarbe / Portalfarbe konfiguriert werden. Von dieser `portalColor` werden viele der weiteren Farbstyles für die Anwendung abgeleitet und in der generierten Datei `color.css` abgelegt, die in der Zielanwendung einzubinden ist.
