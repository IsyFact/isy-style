...
* Firefox ESR, jeweils aktuelle Version (Stand Oktober 2018 ESR 60 - s. xref:glossary:literaturextern:inhalt.adoc#litextern-firefox-esr-roadmap[Firefox ESR Roadmap])
...
* Die Position der Dialogbuttons sollten einheitlich über die verschiedenen modalen Dialoge der Anwendung sein. Der modale Dialog in seiner Gesamtkeit darf bei Bedarf scrollbar sein. Eine permanent sichtbare Kopf- und Fußzeile ist nicht verpflichtend.
...
** Nutzt der Benutzer die *Druck-Funktion des Browsers* (Datei > Drucken), dann werden die oben beschriebenen Regeln angewendet, es findet keine weitere Optimierung des Layouts statt.
...
.Toggle-Filter im Detailbereich
...
xref:glossary:literaturextern:inhalt.adoc#litextern-paciellogroup-kontrast-analyse[Paciello Group Kontrast Analyse]
